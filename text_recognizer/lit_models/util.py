...
def first_appearance(x: torch.Tensor, element: Union[int, float], dim: int = 1) -> torch.Tensor:
    ...
    Args:
        x (torch.Tensor): One or two-dimensional Tensor to search for element.
        element (Union[int, float]): Item to search for inside x.
        dim (int, optional): Dimension of Tensor to collapse over.. Defaults to 1.
...
def replace_after(x: torch.Tensor, element: Union[int, float], replace: Union[int, float]) -> torch.Tensor:
    ...
    Args:
        x (torch.Tensor): Two-dimensional Tensor (shape denoted (B, S)) to replace values in.
        element (Union[int, float]): Item to search for inside x.
        replace (Union[int, float]): Item that replaces entries that appear after element.

    Returns:
        torch.Tensor: New Tensor of same shape as x with values after element replaced.
